		    ...
		    <div class="col-12">
		      <div class="card">
		        <div class="card-header">
		          <h5 class="card-title">Blogs Listing</h5>
		           <a href="{{ url('admin/blogs/create')}}" class="btn btn-sm btn-info float-right">Add New Blog</a>
		        </div>
		        <!-- /.card-header -->
		        <div class="card-body">
		            ...
		            <tbody>
		             @foreach($faqs as $index => $banner)
			            <tr>
			              ...
			              <td>{{ $banner->title }}</td>
			              <td>{{ \Illuminate\Support\Str::limit($banner->description, 50, $end='...') }}</td>
			              <td>@if($banner->image) <img height="50px" width="50px" src="{{ Storage::disk('spaces')->url('thumbs/'.$banner->image) }}"> @endif</td>
			              <td>
			              	<a href="{{ url('admin/blogs') .'/'.$banner->id.'/edit'}}" class="btn btn-sm btn-info float-left">Edit</a>
    		...
    		$(".delete-faq").click(function(e){
			          ...
			          Swal.fire({
			            ...
			            if (result.value) {
			                $.ajax({
			                   type:'DELETE',
			                   url:base_url+'/admin/blogs/'+banner_id,
			                   data:{id:banner_id},
