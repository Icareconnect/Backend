...
function updateGroupChannelLastMessage(message) {
  ...
  if (message) {
    ...
    if (lastMessageDateString) {
      lastMessageDateString = '<div><img src="http://mp2r.localhost/consultant_app/public/assets/images/icon-time.png" style="vertical-align: moddle; padding-bottom: 2px;" /> <span> ' + lastMessageDateString + '</span></div>';
    }
...
function addGroupChannel(isGroup, channelMemberList, targetChannel,receiver_chat_bid,receiver_chat_bname) {
  //alert(receiver_chat_bid);
  if (isGroup) {
  ...
  if (addFlag && !isGroup) {
    ...
    $('#messaging_channel_list').append(
      '<div data-id="'+receiver_chat_bid+'"  class="left-nav-channel left-nav-channel-messaging left-nav-channel-messaging--active" ' +
   ...
   var newchannelMemberList=channelMemberList.split(',')[0];
  //alert(receiver_chat_bid);

    $('#messaging_channel_list').append(
      
      '<div id="getrecevicer"  data-id="'+receiver_chat_bid+'"style="width: 100%;margin-left: 0px;cursor: pointer;" class="left-nav-channel left-nav-channel-group left-nav-channel-group--active row d-flex align-items-center" ' +
      '     onclick="joinGroupChannel(\'' + targetChannel.url + '\')"' +
      '     data-channel-url="' + targetChannel.url + '"' +
      '>' +
      '<div class="col-md-3"><img class="circle img-fluid" src="'+base_url+'/assets/mp2r/images/default.png"></div>'+
      '<div class="left-nav-channel-members col-md-9 pl-0">' + receiver_chat_bname + '</div>' +
      '<span class="left-nav-channel-lastmessage" style="margin-left: 80px;word-break: break-word;width: 250px;white-space: nowrap;overflow: hidden;text-overflow: ellipsis;"></span>' +
      
      '<span class="left-nav-channel-lastmessagetime" style="margin-left: auto;padding: 4px;"></span>' +
      '</div>'
    );
...
function getGroupChannelList() {
  GroupChannelListQuery.next(function (channels, error) {
    ...
    channels.forEach(function (channel) {
      ...
      var members = channel.members;
      var  receiver_chat_bid ='';
      var receiver_chat_bname='';

      members.forEach(function (member) {
        if (currentUser.userId != member.userId) {
          channelMemberList += xssEscape(member);
          receiver_chat_bid=member.userId;
          receiver_chat_bname=member.nickname;
        }
      });
      //alert(receiver_chat_bid);
      channelMemberList = channelMemberList.slice(0, -2);
      addGroupChannel(true, channelMemberList, channel,receiver_chat_bid,receiver_chat_bname);
      //yee
...
function startSendBird(userId, nickName) {
  ...
  sb.connect(userId, function (user, error) {
    if (error) {
      ...
      var userIds = [getSearchParams("receiver_id"), getSearchParams("userid")]; 

      sb.GroupChannel.createChannelWithUserIds(userIds, true, 'test_name', '', '', function (channel, error) {
      if (error) {
...
function messageList(message) {
  ...
  if (message.isAdminMessage()) {
    ...
    if (isCurrentUser(user.userId)) {
      ...
      var msg = '' +
        '<div class="offset-md-5 col-md-7 d-flex  chat-canvas__list" >' +
        
        
        '  <div style="max-width: 100%;margin: 7px 0px;padding: 10px 12px;position: relative;width: auto;display: inline-block;text-align: right;margin-left: auto;width: auto;float: right;word-break: break-word;" class="main-chat-block chat-canvas__list-text" data-messageid="%messageid%">%message%</div></div>';
      ...
      var msg = '' +
        '<div class="col-md-7 chat-canvas__list">' +
        
        '  <div class="secondary-chat-block  chat-canvas__list-text" data-messageid="%messageid%">' +
...
function imageMessageList(obj) {
  ...
  if (isCurrentUser(user.userId)) {
    msgList += '' +
      ...
      '  <div style="background-color: unset;" class="main-chat-block chat-canvas__list-file" onclick="window.open(\'' + xssEscape(message.url) + '\', \'_blank\');">' +
    ...
    msgList += '' +
      ...
      '  <div class="secondary-chat-block chat-canvas__list-file" onclick="window.open(\'' + xssEscape(message.url) + '\', \'_blank\');">' +
      '    <img src="' + xssEscape(message.url) + '" class="chat-canvas__list-file-img" onload="afterImageLoad(this)">' +
      '  </div>' +
      '</div>';
  }
...
function addUnreadCount(item, unread, targetUrl) {
  ...
  if ($(item).find('div[class="left-nav-channel__unread"]').length != 0) {
    ...
    $('<div class="left-nav-channel__unread"></div>')
...
$(document).on('click','#getrecevicer',function(){


        $("#homesecond").removeClass('show active');

        $("#home").addClass('show active');

        $(".left-nav-channel-group--active").removeClass('left-nav-channel-group--active');

        $(this).addClass('left-nav-channel-group--active');
