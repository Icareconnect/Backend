...
function addGroupChannel(isGroup, channelMemberList, targetChannel) {
  ...
  if (addFlag && !isGroup) {
    ...
    $('#messaging_channel_list').append(
      ...
      '<div style="width: 100%;margin-left: 0px;cursor: pointer;" class="left-nav-channel left-nav-channel-group left-nav-channel-group--active row d-flex align-items-center" ' +
      '     onclick="joinGroupChannel(\'' + targetChannel.url + '\')"' +
      '     data-channel-url="' + targetChannel.url + '"' +
      '>' +
      '<div class="col-md-3"><img class="circle img-fluid" src="http://mp2r.localhost/consultant_app/public/assets/images/patient-img.png"></div>'+
      '<div class="left-nav-channel-members col-md-9 pl-0">' + channelMemberList + '</div>' +
      '<span class="left-nav-channel-lastmessage" style="margin-left: 80px;"></span>' +
      '<span class="left-nav-channel-lastmessagetime" style="margin-left: 80px;"></span>' +
      '</div>'
    );
...
function messageList(message) {
  ...
  if (message.isAdminMessage()) {
    ...
    if (isCurrentUser(user.userId)) {
      ...
      var msg = '' +
        '<div class="offset-md-5 col-md-7  chat-canvas__list" style="margin-left: 68.666667%;">' +
        
        
        '  <div class="main-chat-block chat-canvas__list-text" data-messageid="%messageid%">%message%</div></div>';
...
function imageMessageList(obj) {
  ...
  if (isCurrentUser(user.userId)) {
    msgList += '' +
      ...
      '  <div style="margin-left: 105.666667%;background-color: unset;" class="main-chat-block chat-canvas__list-file" onclick="window.open(\'' + xssEscape(message.url) + '\', \'_blank\');">' +
      '    <img style="width: 104px;" src="' + xssEscape(message.url) + '" class="chat-canvas__list-file-img" onload="afterImageLoad(this)">' +
      '  </div>' +
      '</div>';
  } else {
    msgList += '' +
      ...
      '  <div style="margin-left: 20px;" class="secondary-chat-block chat-canvas__list-file" onclick="window.open(\'' + xssEscape(message.url) + '\', \'_blank\');">' +
      '    <img  style="width: 100px;" src="' + xssEscape(message.url) + '" class="chat-canvas__list-file-img" onload="afterImageLoad(this)">' +
      '  </div>' +
      '</div>';
  }
